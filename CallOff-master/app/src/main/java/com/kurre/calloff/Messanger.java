                ...
                public void run() {
                    while(!Thread.currentThread().isInterrupted())
                    {
                        ...
                        if (message.messageType == Header.MESSAGE) {
                            RecentChat.vibrator.vibrate(250);
                            final String response = read_message(message.messageLength);
                            message.message = response;
                            myDbHelper.insertMessage(message);
                            if (RecentChat.contactPage != null)
                                RecentChat.contactPage.refreshContactChat();
                            ...
                            if(RecentChat.callActivity != null)
                                RecentChat.callActivity.endCall();
                        } else if (message.messageType == Header.CALL_CONNECTED) {
                            if(RecentChat.callActivity != null)
                                RecentChat.callActivity.startCall();
